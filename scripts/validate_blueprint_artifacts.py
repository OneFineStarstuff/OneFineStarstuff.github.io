...
def validate_json_manifest(base_dir: Path = DEFAULT_ART) -> ValidationResult:
    ...
    if not isinstance(manifest["artifacts"], list) or not manifest["artifacts"]:
        return ValidationResult(
            "manifest_structure", False, "Manifest artifacts must be a non-empty list"
        )
    for item in manifest["artifacts"]:
        required = {"name", "location", "hash", "signature"}
        if set(item.keys()) != required:
            return ValidationResult(
                "manifest_structure", False, f"Invalid artifact item keys: {item}"
            )
        for key in required:
            if not isinstance(item[key], str) or not item[key].strip():
                return ValidationResult(
                    "manifest_structure",
                    False,
                    f"Artifact field '{key}' must be non-empty",
                )
...
def validate_manifest_timestamp(base_dir: Path = DEFAULT_ART) -> ValidationResult:
    ...
    try:
        ...
        return ValidationResult(
            "manifest_timestamp", False, f"Invalid generated_at timestamp: {ts}"
        )
    return ValidationResult(
        "manifest_timestamp", True, "generated_at timestamp is valid ISO-8601"
    )
...
def validate_schema_metadata(base_dir: Path = DEFAULT_ART) -> ValidationResult:
    ...
    if schema.get("type") != "object":
        return ValidationResult(
            "schema_metadata", False, "Top-level schema type must be object"
        )
    return ValidationResult("schema_metadata", True, "Schema metadata is valid")


def validate_manifest_against_schema_contract(
    base_dir: Path = DEFAULT_ART,
) -> ValidationResult:
    schema = load_schema(base_dir)
    manifest = load_manifest(base_dir)
    ...
    return ValidationResult(
        "schema_contract", True, "Manifest matches schema contract keys"
    )
...
def validate_schema_constraints(base_dir: Path = DEFAULT_ART) -> ValidationResult:
    ...
    for key in schema.get("required", []):
        if key not in manifest:
            return ValidationResult(
                "schema_constraints", False, f"Missing required field: {key}"
            )
    ...
    for key in ("manifest_id", "generated_at", "institution"):
        ...
        if not isinstance(value, str) or len(value) < min_len:
            return ValidationResult(
                "schema_constraints", False, f"Field {key} violates minLength/type"
            )
    ...
    if not isinstance(artifacts, list) or len(artifacts) < min_items:
        return ValidationResult(
            "schema_constraints", False, "artifacts violates minItems/type"
        )
    ...
    for idx, item in enumerate(artifacts, start=1):
        for req in item_required:
            if req not in item:
                return ValidationResult(
                    "schema_constraints", False, f"artifacts[{idx}] missing field {req}"
                )
        for req in item_required:
            ...
            if not isinstance(value, str) or len(value) < min_len:
                return ValidationResult(
                    "schema_constraints",
                    False,
                    f"artifacts[{idx}].{req} violates minLength/type",
                )

    return ValidationResult(
        "schema_constraints", True, "Manifest satisfies schema keyword constraints"
    )
...
def validate_csv_headers(base_dir: Path = DEFAULT_ART) -> ValidationResult:
    ...
    if len(crosswalk_rows) < 2:
        return ValidationResult(
            "csv_semantics", False, "Crosswalk CSV must include at least one data row"
        )
    ...
    if headers != expected:
        return ValidationResult(
            "csv_semantics", False, "Model risk register CSV header mismatch"
        )
    ...
    for idx, row in enumerate(rows[1:], start=2):
        if len(row) != len(expected):
            return ValidationResult(
                "csv_semantics", False, f"Model risk row {idx} has wrong column count"
            )
        ...
        if risk_tier not in allowed_tiers:
            return ValidationResult(
                "csv_semantics",
                False,
                f"Model risk row {idx} has invalid risk_tier: {row[3]}",
            )
        if not _valid_date(row[5]) or not _valid_date(row[7]):
            return ValidationResult(
                "csv_semantics", False, f"Model risk row {idx} has invalid date format"
            )

    return ValidationResult(
        "csv_semantics", True, "CSV headers and sample rows are valid"
    )
...
def validate_rego_guardrails(base_dir: Path = DEFAULT_ART) -> ValidationResult:
    ...
    if missing:
        return ValidationResult(
            "rego_guardrails", False, f"Missing Rego guardrails: {missing}"
        )
...
def validate_yaml_examples(base_dir: Path = DEFAULT_ART) -> ValidationResult:
    kafka_obj = yaml.safe_load(
        (base_dir / "T8_Kafka_Audit_ACL_Example.yaml").read_text()
    )
    k8s_obj = yaml.safe_load(
        (base_dir / "T9_K8s_NetworkPolicy_Example.yaml").read_text()
    )

    if not isinstance(kafka_obj, dict):
        return ValidationResult(
            "yaml_examples", False, "Kafka ACL YAML must be a mapping"
        )
    if "principals" not in kafka_obj or "constraints" not in kafka_obj:
        return ValidationResult(
            "yaml_examples", False, "Kafka ACL YAML missing principals/constraints"
        )
    if (
        not isinstance(kafka_obj["principals"], list)
        or len(kafka_obj["principals"]) == 0
    ):
        return ValidationResult(
            "yaml_examples", False, "Kafka ACL YAML principals must be a non-empty list"
        )

    constraints = kafka_obj.get("constraints", [])
    if not any(
        isinstance(item, dict) and item.get("enforce_mtls") is True
        for item in constraints
    ):
        return ValidationResult(
            "yaml_examples",
            False,
            "Kafka ACL YAML missing enforce_mtls: true constraint",
        )
    ...
    if k8s_obj.get("kind") != "NetworkPolicy":
        return ValidationResult(
            "yaml_examples", False, "K8s YAML kind must be NetworkPolicy"
        )
    ...
    if "Egress" not in policy_types:
        return ValidationResult(
            "yaml_examples", False, "K8s YAML policyTypes must include Egress"
        )

    return ValidationResult(
        "yaml_examples", True, "YAML examples parse and satisfy required semantics"
    )


def safe_run(
    name: str, fn: Callable[[Path], ValidationResult], base_dir: Path
) -> ValidationResult:
    try:
        return fn(base_dir)
    except Exception as exc:  # defensive: convert unexpected errors into check failures
        return ValidationResult(
            name, False, f"Unhandled exception: {type(exc).__name__}: {exc}"
        )
...
def run_validations(base_dir: Path = DEFAULT_ART) -> list[ValidationResult]:
    ...
    for name, _desc in CHECK_SEQUENCE:
        if name in seen:
            results.append(
                ValidationResult(
                    name, False, f"Duplicate check ID in CHECK_SEQUENCE: {name}"
                )
            )
            continue
        seen.add(name)
        ...
        if fn is None:
            results.append(
                ValidationResult(
                    name, False, f"No handler registered for check ID: {name}"
                )
            )
            continue
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate blueprint artifact starter pack"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--base-dir",
        type=str,
        default=str(DEFAULT_ART),
        help="Override artifact directory",
    )
    parser.add_argument(
        "--list-checks", action="store_true", help="List stable check IDs and exit"
    )
