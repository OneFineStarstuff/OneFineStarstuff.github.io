...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build or check artifact checksum manifest"
    )
    parser.add_argument(
        "--check", action="store_true", help="Exit non-zero if manifest is out of date"
    )
    parser.add_argument(
        ...
        if args.check:
            if not out.exists():
                emit(
                    ...
                    {
                        "status": "error",
                        "message": "Manifest missing. Run: python artifacts/build_manifest.py",
                    },
                )
                return 1
            try:
                ...
                emit(
                    ...
                    {
                        "status": "error",
                        "message": "Manifest file is invalid JSON. Run: python artifacts/build_manifest.py",
                    },
                )
                return 1
            if not isinstance(existing, dict):
                emit(
                    ...
                    {
                        "status": "error",
                        "message": "Manifest file has invalid structure. Run: python artifacts/build_manifest.py",
                    },
                )
                return 1
            if existing.get("version") != payload.get("version") or existing.get(
                "files"
            ) != payload.get("files"):
                emit(
                    args,
                    {
                        "status": "error",
                        "message": "Manifest is out of date. Run: python artifacts/build_manifest.py",
                    },
                )
                return 1
