...
echo "[1/15] OPA policy tests (release gate + credit + attestation/PCR_MATCH)"
...
echo "[2/15] TLA+ TLC model check (KillSwitchAbstract — con-04/con-07)"
...
echo "[3/15] TLA+ TLC model check (AdmissionWithAttestation — env-01)"
if java -cp "$GA/tla/tools/tla2tools.jar" tlc2.TLC \
...
echo "[4/15] TLA+ TLC model check (SentinelContainmentProtocol — dead-man's switch)"
if java -cp "$GA/tla/tools/tla2tools.jar" tlc2.TLC \
...
echo "[5/15] GC-IR cross-target conformance (Rego <=> circuit <=> expectation)"
...
echo "[6/15] SRC-1 Groth16 proof flow (cry-05 concentration bound)"
...
echo "[7/15] zk-SNARK relayer pipeline (Solidity Groth16 verifier + calldata)"
...
echo "[8/15] SARA/ACR MoE routing stabilization (rte-01)"
...
echo "[9/15] PQC WORM audit log (ML-DSA-65 / CRYSTALS-Dilithium — cry-02)"
...
echo "[10/15] Solidity compile + OmegaActual hardening logic (SEC-01..06)"
...
echo "[11/15] Governance artifact schema validation"
...
echo "[12/15] OSCAL catalog conformance (prop/href cross-reference integrity)"
...
echo "[13/15] Annex IV dossier auto-assembly (8 sections from conformant catalog)"
...
echo "[14/15] DORA ICT-risk register auto-assembly (5 pillars; gaps reported)"
...
echo "[15/15] NIST AI RMF profile crosswalk auto-assembly (GOVERN/MAP/MEASURE/MANAGE)"
