...
if HMAC_SECRET == "<REDACTED_SECRET>":
    print("[WARN] Using default HMAC key. Set OMNI_SENTINEL_HMAC_KEY env variable.", file=sys.stderr)
...
class RuleEngine:
    ...
    def evaluate(self, telemetry: TelemetrySnapshot) -> Tuple[Optional[Rule], List[Rule]]:
        """
    ...
    def _log_conflict_resolution(self, telemetry: TelemetrySnapshot,
                                  triggered: List[Rule], winner: Rule):
...
class VisualizationEngine:
    ...
    def render_resource_summary(snapshot: TelemetrySnapshot):
        ...
        lines.append("="*80)
        lines.append(f"  Timestamp:       {datetime.fromtimestamp(snapshot.timestamp).isoformat()}")
        ...
        lines.append(f"  Latency:         {snapshot.latency_ms:6.2f} ms ({snapshot.latency_blocks} blocks)")
        lines.append("="*80 + "\n")
        return "\n".join(lines)
    ...
    def render_phase_state(phase: PhaseState, triggered_rules: List[Rule]):
        ...
        if triggered_rules:
            ...
            for rule in triggered_rules:
                lines.append(f"    - [{rule.action.name:12s}] {rule.name} (Priority: {rule.priority})")
        else:
...
class OmniSentinel:
    ...
    def _execute_halt(self, rule: Rule, snapshot: TelemetrySnapshot):
        ...
        if self.phase != PhaseState.HALTED:
            self._log_phase_transition(
                PhaseState.HALTED,
                f"HALT triggered by rule: {rule.name}"
            )
            print(f"\n{'!'*80}")
    ...
    def _execute_override(self, rule: Rule, snapshot: TelemetrySnapshot):
        ...
        if self.phase == PhaseState.MONITORING:
            self._log_phase_transition(
                PhaseState.ALERT,
                f"OVERRIDE triggered by rule: {rule.name}"
            )
