#!/usr/bin/env python3
"""WP-062 HTML renderer — Civilizational AGI/ASI Master Synthesis Blueprint 2026-2030."""
import json
from pathlib import Path
from html import escape

ROOT = Path(__file__).resolve().parent
SRC = ROOT / "data" / "civ-agi-master-synthesis-2030.json"
OUT = ROOT / "public" / "civ-agi-master-synthesis-2030.html"
...
SKIP = (
    ...
    "iid", "fid", "rsid", "name", "layer", "component", "system", "area", "category",
    "mechanism", "tier", "control", "phase", "milestone", "regime", "clause",
    "capability", "artifact", "scope", "item", "practice", "invariant", "prover",
    "from", "to", "plane", "abstract", "content",
)
...
# Distinctive arrays specific to WP-062
distinctive = [
    ("refArchLayers",        "ref-arch-layers",       "Reference Architecture Layers (M3)",            ["rid", "system", "layer"]),
    ("platformLayers",       "platform-layers",       "Governance Platform Layers (M7/M8)",            ["pid", "plane", "component"]),
    ("regulatoryCrosswalks", "regulatory-crosswalks",  "Regulatory Crosswalks — 30+ Regimes (M2)",     ["cid", "regime", "clause"]),
    ("safetyInvariants",     "safety-invariants",      "Luminous Engine Codex Safety Invariants (M4)", ["iid", "prover", "invariant"]),
    ("frontierRisks",        "frontier-risks",         "Frontier-Risk Taxonomy (M4)",                  ["fid", "category"]),
    ("civMechanisms",        "civ-mechanisms",         "Civilizational Coordination Mechanisms (M5)",  ["mid", "name"]),
    ("reportSections",       "report-sections",        "Regulator-Ready Report Sections (M8)",         ["rsid", "title"]),
    ("roadmap",              "roadmap-items",          "2026-2030 Roadmap Items (M8)",                 ["rid", "phase", "milestone"]),
...
# Report sections rendered with abstract/content (whitepaper style)
report_full_html = (
...
code_html = (
    "<section id='code'><h3>Code &amp; Skeleton Artifacts (Rego / TLA+ / Coq / Q# / Terraform / Kafka ACL)</h3>"
    + "".join(
...
# Meta / directive / regimes / indices / tiers / severities / investment / audiences
...
<section id='tiers'><h3>AI Risk / Containment Tiers T0-T4</h3><ul>{tiers_rows}</ul></section>

<section id='severities'><h3>Incident Severity Levels</h3><ul>{sev_rows}</ul></section>
...
<li><a href='#tiers'>Risk/Containment Tiers</a></li>
...
print(f"WP-062 HTML written: {OUT} ({OUT.stat().st_size} bytes)")
