#!/usr/bin/env python3
"""WP-053 — AGI-GOVERNANCE-MASTER-BLUEPRINT HTML dashboard renderer."""
import json, html
from pathlib import Path

ROOT = Path(__file__).parent
SRC = ROOT / "data" / "agi-governance-master-blueprint.json"
OUT = ROOT / "public" / "agi-governance-master-blueprint.html"
...
# Distinctive WP-053 — Appendix Templates + Checklists
template_html = "".join(
    f"<details class='code' id='{esc(t['id'])}'><summary><b>{esc(t['id'])}</b> — {esc(t['name'])} <i>(Owner: {esc(t.get('owner',''))})</i></summary>"
    f"<p class='summary'><b>Purpose:</b> {esc(t.get('purpose',''))}</p>"
    f"<h5>Fields ({len(t.get('fields',[]))})</h5>"
    f"{render_list(t.get('fields',[]))}"
    f"</details>"
    for t in D["appendixTemplates"]
)
checklist_html = "".join(
    f"<details class='code' id='{esc(c['id'])}'><summary><b>{esc(c['id'])}</b> — {esc(c['name'])} <i>({esc(c.get('frequency',''))})</i></summary>"
    f"<p class='summary'><b>Scope:</b> {esc(c.get('scope',''))}</p>"
    f"<h5>Items ({len(c.get('items',[]))})</h5>"
    f"{render_list(c.get('items',[]))}"
    f"</details>"
    for c in D["appendixChecklists"]
)
...
<nav>
  ...
  <a href="#code">Code</a>
  <a href="#templates">Appendix: Templates</a>
  <a href="#checklists">Appendix: Checklists</a>
...
<section class='block' id='templates'>
  <h2>Appendix A — Templates ({len(D['appendixTemplates'])}) — TPL-A..TPL-H</h2>
  <p class='summary'>Distinctive WP-053 element: ready-to-deploy templates for Annex IV, FRIA, DPIA, Conflict Register, Board AI Charter, Incident Report, Model Card v2, Vendor Due Diligence — each owner-assigned and field-itemised for legal defensibility.</p>
  {template_html}
</section>

<section class='block' id='checklists'>
  <h2>Appendix B — Checklists ({len(D['appendixChecklists'])}) — CHK-1..CHK-7</h2>
  <p class='summary'>Distinctive WP-053 element: operational checklists for Pre-Deploy, Quarterly Review, Annual Attestation, Incident Response, Frontier Run, Auditor Evidence-Pack Prep, and Supervisor Exam Rehearsal — each with scope, items, and frequency for auditable compliance.</p>
  {checklist_html}
</section>
