...
def main():
    ...
    n_rsp_versions = len(
        data.get("M4_rsp", {}).get("sections", [{}])[0].get("versions", [])
    )
    n_phases = len(
        data.get("M10_roadmap", {}).get("sections", [{}])[0].get("phases", [])
    )
    n_kpis = len(
        data.get("M10_roadmap", {}).get("sections", [{}, {}])[1].get("kpis", [])
    )
...
<header class="hero">
  ...
  <div class="badges">
    ...
    <span class="badge warn">EU AI Act Annex III §5(b)</span>
    <span class="badge warn">GDPR Art. 22</span>
    <span class="badge">SR 11-7 / OCC 2011-12</span>
    <span class="badge">PRA SS1/23</span>
    <span class="badge">ECB SSM</span>
    <span class="badge crit">FCRA / ECOA</span>
    <span class="badge ok">RSP v2.6 ready</span>
  </div>
  <div class="kpis">
    ...
    <div class="kpi"><div class="v">5</div><div class="l">AIMS Sections (1–5)</div></div>
    <div class="kpi"><div class="v">4</div><div class="l">Annexes (J1–J4)</div></div>
    <div class="kpi"><div class="v">{n_overlays}</div><div class="l">Regulatory Overlays</div></div>
    <div class="kpi"><div class="v">{n_rsp_versions}</div><div class="l">RSP Versions</div></div>
    <div class="kpi"><div class="v">{n_phases}</div><div class="l">Phases</div></div>
    <div class="kpi"><div class="v">{n_kpis}</div><div class="l">KPIs</div></div>
...
<main>
  ...
  <section class="module" id="schemas">
    <h2>JSON Schemas</h2>
    <p class="summary">{n_schemas} schemas: AI System Inventory, RSP Manifest, Decision Envelope, Control Mapping, FRIA, Incident Record, FedReg Message, Obligation Spec.</p>
    {schemas_html}
  </section>

  <section class="module" id="code-examples">
    <h2>Code Examples</h2>
    <p class="summary">{n_code} reference implementations: OPA RSP gate, Terraform WORM evidence, decision envelope signer (Ed25519 + PQC), fairness monitor, FedReg client, predictive drift forecaster, TLA+ obligation spec, Lean FCRA spec, self-healing playbook engine, FastAPI traceability API, Merkle anchor.</p>
    {code_html}
  </section>

  <section class="module" id="case-studies">
    <h2>Case Studies</h2>
    <p class="summary">{n_cs} reference deployments: EU G-SIB dual ISO/EU AI Act certification, US BHC federated SR 11-7+EU AI Act, UK PRA SS1/23 SMF24 attestation, joint ECB+Fed+PRA examination, self-healing bias drift auto-rollback.</p>
    {cs_html}
  </section>
