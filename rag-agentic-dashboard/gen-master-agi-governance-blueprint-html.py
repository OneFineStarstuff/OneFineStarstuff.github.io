#!/usr/bin/env python3
"""WP-061 HTML renderer — Master AGI/ASI Governance Blueprint 2026-2030 (G-SIFI)."""
import json
from pathlib import Path
from html import escape

ROOT = Path(__file__).resolve().parent
SRC = ROOT / "data" / "master-agi-governance-blueprint.json"
OUT = ROOT / "public" / "master-agi-governance-blueprint.html"
...
SKIP = (
    "mid", "sid", "title", "pid", "cid", "rid", "aid", "did", "eid", "uid", "yid",
    "name", "layer", "component", "system", "area", "category", "mechanism", "tier",
    "control", "phase", "milestone", "regime", "clause", "capability", "artifact",
    "scope", "item", "practice", "invariant", "prover", "from", "to", "plane",
)
...
def module_html(m):
    ...
    purpose = m.get("purpose") or m.get("summary") or ""
    return (
        f"<section class='module' id='{e(m['mid'])}'>"
        f"<h3>{e(m['mid'])} — {e(m['title'])}</h3>"
        f"<p class='sum'>{e(purpose)}</p>"
        f"{secs}</section>"
    )
...
# 11 distinctive arrays
distinctive = [
    ("refArchLayers",        "ref-arch-layers",        "Reference Architecture Layers (M1)",       ["rid", "system", "layer"]),
    ("platformLayers",       "platform-layers",        "Platform Layers (M2)",                     ["pid", "plane", "component"]),
    ("regulatoryCrosswalks", "regulatory-crosswalks",  "Regulatory Crosswalks (M3)",               ["cid", "regime", "clause"]),
    ("containmentMechanisms","containment-mechanisms", "Containment Mechanisms T0-T4 (M4)",        ["mid", "tier", "mechanism"]),
    ("umifInvariants",       "umif-invariants",        "UMIF Invariants — TLA+/Coq/Q# (M4)",       ["uid", "prover", "invariant"]),
    ("supervisoryLayers",    "supervisory-layers",     "Supervisory & Reporting Layers (M5)",      ["sid", "layer", "capability"]),
    ("annexIVArtifacts",     "annex-iv-artifacts",     "Annex IV Conformity Artifacts (M3/M5)",    ["aid", "artifact", "scope"]),
    ("strategyItems",        "strategy-items",         "Enterprise AI Strategy Items (M6)",        ["eid", "area", "item"]),
    ("roadmapItems",         "roadmap-items",          "2026-2030 Roadmap Items (M7)",             ["rid", "phase", "milestone"]),
    ("systemicPractices",    "systemic-practices",     "Systemic-Risk & Civilizational Practices (M8)", ["yid", "category", "practice"]),
    ("dependencies",         "dependencies",           "Dependency Graph (DAG edges)",             ["did", "from", "to"]),
]
...
def table_dict(d, key_label, val_label="value"):
    """Render dict {k: v} or {k: {fields}} as a 2/3-col table."""
...
roadmap_html  = f"<section id='roadmap'><h3>2026-2030 Roadmap ({len(DOC['roadmap'])})</h3>{table_rows(DOC['roadmap'], ['rid','phase','milestone'])}</section>"
evidence_html = (
...
tail_html = schemas_html + code_html + kpis_html + rcm_html + trace_html + flows_html + regs_html + rollout_html + roadmap_html + evidence_html
...
# Meta / directive / regimes / indices / tiers / investment
directive = DOC["directive"]
indices_rows = "".join(
    f"<li><b>{e(k)}</b>: {e(v) if not isinstance(v, dict) else e(json.dumps(v))}</li>"
    for k, v in DOC["indices"].items()
)
...
<section id='indices'><h3>Performance & Civilizational Indices ({len(DOC['indices'])})</h3><ul>{indices_rows}</ul></section>

<section id='tiers'><h3>Containment Tiers T0-T4</h3><ul>{tiers_rows}</ul></section>
...
<p><b>Uplift vs WP-060:</b> {e(invest['uplift_vs_WP060'])}</p>
...
pre {{ background:#0a0f15; border:1px solid var(--line); border-radius:4px; padding:8px; font-size:11px; overflow-x:auto; color:#d0d7de; margin:0; }}
...
<li><a href='#tiers'>Containment Tiers</a></li>
...
<li><a href='#code'>Code Skeletons</a></li>
...
print(f"WP-061 HTML written: {OUT} ({OUT.stat().st_size} bytes)")
