#!/usr/bin/env python3
"""WP-047 — INST-AGI-MASTER-REF HTML dashboard renderer."""
import json, html
from pathlib import Path

ROOT = Path(__file__).parent
SRC = ROOT / "data" / "inst-agi-master-ref.json"
OUT = ROOT / "public" / "inst-agi-master-ref.html"
...
rollout_rows = "".join(
    f"<tr><td>Day {esc(r['day'])}</td><td>{esc(r['track'])}</td><td>{render_value(r['items'])}</td></tr>"
    for r in D["rollout90"]
)
roadmap_rows = "".join(
    f"<tr><td>{esc(r['year'])}</td><td>{esc(r['focus'])}</td><td>{render_value(r['milestones'])}</td></tr>"
    for r in D["roadmap"]
)
...
<style>
  ...
  * {{ box-sizing:border-box }}
  ...
  .directive pre {{ max-height:260px }}
...
<section class='block' id='rollout'>
  ...
  <table><thead><tr><th>Window</th><th>Track</th><th>Items</th></tr></thead><tbody>{rollout_rows}</tbody></table>
</section>

<section class='block' id='roadmap'>
  <h2>2026-2030 Multi-Year Roadmap ({len(D['roadmap'])} years)</h2>
  <table><thead><tr><th>Year</th><th>Focus</th><th>Milestones</th></tr></thead><tbody>{roadmap_rows}</tbody></table>
</section>

<section class='block' id='artifacts'>
  <h2>Machine-Readable Artifacts by Audience</h2>
