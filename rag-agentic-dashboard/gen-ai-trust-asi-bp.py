...
modules.append(
    {
        ...
        "sections": [
            ...
            section(
                "M10-S2",
                "Trading Agent Review (AlphaTrade-V9)",
                {
                    "checks": [
                        "latent drift",
                        "reward hacking",
...
modules.append(
    {
        ...
        "sections": [
            ...
            section(
                "M14-S3",
                "Federated Learning + GDPR Sovereignty",
                {
            ...
            section(
                "M14-S4",
                "Machine Unlearning (GDPR Art 17)",
                {
                    "techniques": [
                        "SISA training",
            ...
            section(
                "M14-S5",
                "Sleeper-Agent Defense + ASI Honeypot + Deceptive Alignment",
                {
...
code = [
    {
        "id": "CE-01",
        "title": "GitHub Actions — Sigstore + ML-DSA-44 sign",
        "lang": "yaml",
    ...
    {
        "id": "CE-04",
        "title": "Sentinel sidecar — Kafka WORM producer (Go)",
        "lang": "go",
        "snippet": "func (s *Sidecar) Emit(env Envelope) error {\n    body, _ := json.Marshal(env)\n    msg := &kafka.Message{ Topic: &decisionTopic, Key: []byte(env.SystemId), Value: body }\n    return s.producer.Produce(msg, nil)\n}\n",
    },
    {
        "id": "CE-05",
        "title": "eBPF — TC egress redaction (libbpf)",
        "lang": "c",
    ...
    {
        "id": "CE-11",
        "title": "Gradient anomaly (z-score) defense",
        "lang": "python",
...
cases = [
    {
        "id": "CS-01",
        "name": "G-SIB credit AI — auto Annex IV pack rollout",
        "outcomes": "Pack assembly 26 min p95; 0 critical errors; supervisor sign-off month 3",
    },
    {
        "id": "CS-02",
        "name": "AlphaTrade-V9 tabletop — board exercise",
        "outcomes": "Kill-switch p95 53 s; regulator-notify draft within 90 min; board comms clarity score 4.6/5",
    },
    {
        "id": "CS-03",
        "name": "Sleeper-agent FL defense",
        "outcomes": "Detection rate 99.2 % at z ≥ 3.5; FP < 0.5 %",
    },
    {
        "id": "CS-04",
        "name": "ASI honeypot pilot",
        "outcomes": "3 SEV-0 candidates in 6 months; 0 production reach; full forensic capture",
    },
    {
        "id": "CS-05",
        "name": "PQC ML-DSA-44 hybrid migration",
        "outcomes": "100 % WORM + AI BoM coverage by month 9; cutover plan to 2029",
    },
    {
        "id": "CS-06",
        "name": "Machine unlearning Art 17 SLA",
        "outcomes": "Median 11 days; certified eval delta within bounds",
    },
]
...
kpis = [
    ...
    {
        "id": "KPI-14",
        "name": "Replay byte-identical (deterministic)",
        "target": "≥ 99.9 %",
    },
    {
        "id": "KPI-15",
        "name": "Sigstore + ML-DSA-44 coverage",
        "target": "100 % T1 by Day 90",
    },
    ...
    {
        "id": "KPI-20",
        "name": "AI capital buffer attestation",
    ...
    {
        "id": "KPI-23",
        "name": "PQC migration coverage Tier-1",
        "target": "100 % by 2029",
    },
...
riskControlMatrix = [
    ...
    },
    {
        ...
        "controls": ["allow-list validators", "WORM-logged outputs"],
        "kpis": ["KPI-01"],
    },
    {
        "id": "RC-03",
        "threat": "Training data poisoning (LLM03)",
        "controls": ["AI BoM dataset lineage", "Sigstore", "FL gradient anomaly"],
        "kpis": ["KPI-17", "KPI-22"],
    },
    {
        "id": "RC-04",
        "threat": "Model DoS (LLM04)",
        "controls": ["rate limit", "loss-limit on agents"],
        "kpis": ["KPI-04"],
    },
    {
        "id": "RC-05",
        "threat": "Supply chain (LLM05)",
    ...
    },
    {
        ...
        "controls": ["DLP", "eBPF redaction", "RAG ACL"],
        "kpis": ["KPI-01"],
    },
    {
        "id": "RC-07",
    ...
    {
        "id": "RC-09",
        "threat": "Sleeper-agent / backdoor",
        "controls": [
            "gradient anomaly z ≥ 3.5",
            "trigger-set evals",
            "watermark check",
        ],
        "kpis": ["KPI-17"],
    },
    {
        ...
        "controls": ["FL secure aggregation", "per-region keys", "SCCs"],
        "kpis": ["KPI-01"],
    },
    {
        "id": "RC-11",
        "threat": "Tampering with audit trail",
        "controls": ["Object Lock", "daily Merkle", "PQC signing"],
        "kpis": ["KPI-13"],
    },
    {
        "id": "RC-12",
        "threat": "Excess capital under-provision",
        "controls": ["AI capital buffer", "stress test", "Pillar 3 disclosure"],
        "kpis": ["KPI-20"],
    },
]
...
traceability = [
    {
        "feature": "M1 CI/CD signed pipeline",
        "control": "Sigstore + ML-DSA-44 + OPA gate",
        "regimes": ["EU AI Act Art 15", "ISO 42001 Cl 8", "SLSA L3+", "FIPS 204"],
    },
    {
        "feature": "M2 Kafka WORM + replay",
        "control": "Hash-chain + Merkle + deterministic replay",
        "regimes": ["EU AI Act Art 12", "SR 11-7 outcome analysis", "DORA audit"],
    },
    {
        "feature": "M3 zero-egress K8s",
        "control": "Cilium + Kata + Gatekeeper",
        "regimes": ["DORA ICT", "CIS K8s", "GDPR Art 32"],
    },
    {
        "feature": "M4 React dashboards",
        "control": "CSP + WebAuthn + RBAC",
        "regimes": ["WCAG 2.2", "ISO 27001"],
    },
    {
        "feature": "M5 RAG fiduciary",
        "control": "cosine ≥ 0.92 + Judge LLM",
        "regimes": ["FCA Consumer Duty", "MAS FEAT", "EU AI Act Art 13"],
    },
    {
        "feature": "M6 Annex IV / SR 11-7 pack",
        "control": "Auto-assembly + PAdES + Sigstore",
        "regimes": ["EU AI Act Annex IV", "SR 11-7"],
    },
    {
        "feature": "M7 SEV matrix + tabletop",
        "control": "Multisig kill + regulator notify",
        "regimes": ["EU AI Act Art 73", "PRA SS1/23", "FCA P11"],
    },
    {
        "feature": "M8 2LoD red-team",
        "control": "Polymorphic attack + Judge LLM",
        "regimes": ["EU AI Act Art 15", "NIST GAI Profile"],
    },
    {
        "feature": "M9 Compute consortium + buffer",
        "control": "Registry + AI capital buffer",
        "regimes": ["Basel Pillar 2", "FSB AI"],
    },
    {
        "feature": "M10 trading + credit reviews",
        "control": "Effective challenge + AI BoM",
        "regimes": ["SR 11-7", "FCRA §615(a)", "ECOA Reg B"],
    },
    {
        "feature": "M11 replay tooling",
        "control": "SHAP + signed receipts",
        "regimes": ["SR 11-7", "EU AI Act Art 12"],
    },
    {
        "feature": "M12 eBPF + BMC",
        "control": "Kernel redaction + physical kill",
        "regimes": ["GDPR Art 32", "DORA ICT"],
    },
    {
        "feature": "M13 guardrail + judge prompts",
        "control": "Structured output + ensemble judge",
        "regimes": ["NIST GAI Profile", "MAS FEAT"],
    },
    {
        "feature": "M14 PQC + FL + unlearning + honeypot",
        "control": "FIPS 204 + DP-SGD + SISA + decoy",
        "regimes": ["GDPR Art 17", "FIPS 204", "FIPS 203", "OECD AI Principles"],
    },
]
...
dataFlows = [
    {
        ...
        "controls": ["SLSA L3+", "in-toto", "OPA"],
    },
    {
        "id": "DF-02",
        "name": "Inference → WORM → replay",
        "steps": [
            "app → sidecar",
            "OPA decide",
            "PII redact",
        ...
        "controls": ["mTLS", "Object Lock", "PQC", "deterministic seed"],
    },
    {
        "id": "DF-03",
        "name": "SEV-0 → kill-switch",
        "steps": [
            "alert",
            "triage prompt",
            "multisig 3-of-5",
    ...
    },
    {
        ...
        "controls": ["ε ≤ 4", "z ≥ 3.5 quarantine"],
    },
    {
        "id": "DF-06",
        "name": "Honeypot engagement",
        "steps": [
            "agent probes decoy",
            "log engagement",
            "deception score",
        ...
        "controls": ["isolated VLAN", "egress to /dev/null"],
    },
]
...
workshops = [
    {
        ...
        "duration": "2 h",
        "outcome": "Risk appetite + AlphaTrade-V9 tabletop sign-off",
    },
    {
        "id": "WS-02",
        "audience": "MRM + AI Risk",
        "duration": "1 d",
        "outcome": "Trading + credit review playbook",
    },
    {
        "id": "WS-03",
        "audience": "Platform Engineering",
        "duration": "2 d",
        "outcome": "Sentinel + OPA Gatekeeper + Cilium bootcamp",
    },
    {
        "id": "WS-04",
        "audience": "SOC + IR",
        "duration": "1 d",
        "outcome": "SEV-0..SEV-3 runbook drill",
    },
    {
        "id": "WS-05",
        "audience": "Internal Audit (3LoD)",
        "duration": "1 d",
        "outcome": "Replay + WORM verifier inspection",
    },
    {
        "id": "WS-06",
        "audience": "Supervisor liaison",
        "duration": "0.5 d",
        "outcome": "Annex IV pack + demo kit walkthrough",
    },
    {
        "id": "WS-07",
        "audience": "Trading desk + Credit risk",
        "duration": "1 d",
        "outcome": "Adversarial eval + AI BoM workflow",
    },
]
