#!/usr/bin/env python3
"""WP-066 HTML renderer — SIP v2.4, G-SRI, Red Dawn, ASA & 2026-2035 roadmap blueprint."""
import json
from pathlib import Path
from html import escape

ROOT = Path(__file__).resolve().parent
SRC = ROOT / "data" / "sip-gsri-reddawn-2035.json"
OUT = ROOT / "public" / "sip-gsri-reddawn-2035.html"
...
SKIP = (
    "mid", "sid", "spid", "giid", "rdid", "asaid", "raid", "rpid", "rsid",
    "title", "abstract", "content", "phase", "index", "scenario", "agent",
    "regime", "article", "window",
)
...
distinctive = [
    ("sipPhases",          "sip-phases",          "SIP v2.4 Phases (M1)",                     ["spid", "phase", "window"]),
    ("gsriIndices",        "gsri-indices",        "G-SRI Stress-Test Indices (M2)",           ["giid", "index"]),
    ("redDawnScenarios",   "red-dawn-scenarios",  "Red Dawn Crisis Scenarios (M3)",           ["rdid", "scenario", "severity"]),
    ("supervisoryAgents",  "supervisory-agents",  "Autonomous Supervisory Agents (M4)",       ["asaid", "agent"]),
    ("regArticleMappings", "reg-article-mappings", "Article-Level Regulatory Mapping (M5)",   ["raid", "regime", "article"]),
    ("roadmapPhases",      "roadmap-phases",      "2026-2035 Roadmap Phases (M7)",            ["rpid", "window"]),
]
...
code_html = (
    "<section id='code'><h3>Code &amp; Artifacts (Rego / YAML / OSCAL / TLA+ / OpenAPI)</h3>"
    + "".join(
...
<h4>Modules (M1-M8)</h4>
<ul>{toc_modules}</ul>
<h4>Implementation & Assurance</h4>
<ul>{toc_distinct}</ul>
...
print(f"WP-066 HTML written: {OUT} ({OUT.stat().st_size} bytes)")
