...
modules.append(
    {
        ...
        "sections": [
            ...
            section(
                "M14-S2",
                "Critical-Path Dependency Map (excerpt)",
                {
...
schemas = [
    ...
    {
        "id": "criticalPathNode",
        "fields": ["id", "title", "predecessor", "successor", "slackDays", "owner"],
    },
    ...
    {
        "id": "policyArtifact",
        "fields": ["id", "title", "stage", "stakeholders", "ratifiedBy", "wormAnchor"],
    },
    {
        "id": "uiComponent",
        "fields": ["id", "name", "scope", "api", "owner", "storybookId", "e2eId"],
    },
    {
        "id": "reportTemplate",
        "fields": ["id", "regime", "sections", "format", "signing", "sla"],
    },
    {
        "id": "kpiBinding",
        "fields": ["kpiId", "owner", "target", "evidenceQuery", "wormAnchor"],
    },
    {
        "id": "riskRow",
        "fields": [
    ...
    {
        "id": "trackBacklog",
        "fields": ["track", "p0Items", "p1Items", "p2Items", "p3Items", "p4Items"],
    },
    {
        "id": "okrRollup",
        "fields": ["quarter", "tribe", "objectives", "keyResults", "owner", "evidence"],
    },
]
...
code = [
    ...
    {
        "id": "C3",
        "title": "Critical-path computation (CPM, networkx)",
        "lang": "python",
    ...
    {
        "id": "C4",
        "title": "Phase-gate Rego policy (admission for next phase)",
        "lang": "rego",
        "snippet": 'package pmo.phase_gate\n\ndefault allow := false\n\nallow if {\n  input.phase == "P1"\n  data.kpis["killSwitchSeconds"] <= 60\n  data.kpis["opaP99Ms"] <= 4\n  data.evidence["wormLive"] == true\n}\n',
    },
    {
        "id": "C5",
        "title": "Prompt Architect template (Jinja-safe)",
        "lang": "jinja",
...
cases = [
    {
        "id": "CS-01",
        "name": "G-SIB cuts Annex IV pack time from 14 days to 28 min",
        "outcomes": "WP-050 sequenced critical path (CP-02→CP-08→CP-09→RP-01); auto-assembly hit ≤ 30 min by Day 120; passed EU AI Act audit with 0 major NCs.",
    },
    {
        "id": "CS-02",
        "name": "Multi-vendor LLM-judge ensemble eliminates regression escapes",
        "outcomes": "SC-09 ensemble (3 vendors) gating PRs; κ ≥ 0.92 sustained; regression escape rate dropped 78 % within 90 days.",
    },
    {
        "id": "CS-03",
        "name": "Prompt Architect adoption across 11 business units",
        "outcomes": "PA-01..PA-08 GA by Day 150; 4 800 templates versioned; refusal-lattice coverage 100 % Tier-1; complaint rate -22 %.",
    },
    {
        "id": "CS-04",
        "name": "RAG taint propagation neutralises poisoned-corpus attack",
        "outcomes": "RG-05 + RG-07 detected indirect injection from compromised supplier feed; quarantined 2 600 chunks; zero customer impact.",
    },
    {
        "id": "CS-05",
        "name": "SRASE pre-flight saves G-SIFI a SEV-1 supervisor finding",
        "outcomes": "SM-02 composite 0.86 below gate; CAPA closed in 9 days; real audit pass with merit comment.",
    },
    {
        "id": "CS-06",
        "name": "Treaty annex pipeline + zk-SNARK verifier go live",
        "outcomes": "RP-08 + CP-16 GA by P3; 11 monthly attestations signed PQC; Cert Gold achieved Q3-2027.",
    },
]
...
kpis = [
    {
        "id": "K-01",
        "name": "Phase-gate exit on time",
        "target": "100 % for P0; ≥ 90 % for P1-P3",
    },
...
riskControlMatrix = [
    {
        "id": "R-01",
        "threat": "Critical-path slip on Sigstore + PQC chain",
        "controls": [
            "Vendor pre-engagement",
            "Parallel classical fallback",
            "Phase-gate Rego",
        ],
        "kpis": ["K-01", "K-02"],
    },
    {
        "id": "R-02",
        "threat": "Talent shortage in interpretability",
        "controls": [
            "University partnership",
            "Sentinel Lab fellowships",
            "Contracted experts",
        ],
        "kpis": ["K-23"],
    },
    {
        "id": "R-03",
        "threat": "Vendor LLM SLA volatility",
        "controls": ["Multi-vendor + fallback", "Judge ensemble", "Local small-LLM"],
        "kpis": ["K-09", "K-15"],
    },
    {
        "id": "R-04",
        "threat": "Treaty ratification delay",
        "controls": [
    ...
    {
        "id": "R-05",
        "threat": "Phase-gate overload of PMO",
        "controls": [
            "Automation in Rego",
            "WORM-backed evidence query",
            "Quarterly reviews",
        ],
        "kpis": ["K-01"],
    },
    {
        "id": "R-06",
        "threat": "RAG corpus poisoning supply attack",
        "controls": ["Source attestation", "Taint propagation", "Quarantine workflow"],
        "kpis": ["K-12", "K-13"],
    },
    {
        "id": "R-07",
        "threat": "Prompt Architect template sprawl",
        "controls": [
            "Semver + approval workflow",
    ...
    {
        "id": "R-08",
        "threat": "Registry gaps for 3rd-party models",
        "controls": [
            "API-only wrapper",
            "Vendor attestation",
    ...
    {
        "id": "R-09",
        "threat": "Interpretability research stagnation",
        "controls": [
            "External grants",
    ...
    {
        "id": "R-10",
        "threat": "Threat-intel false-positive overload",
        "controls": ["Correlation + dedup", "SLO alert noise budget", "Auto triage"],
        "kpis": ["K-15"],
    },
    {
        "id": "R-11",
        "threat": "Civilizational sim drift from reality",
        "controls": [
            "AISI joint scenarios",
    ...
    {
        "id": "R-12",
        "threat": "Report-generation correctness regressions",
        "controls": [
            "Golden-set tests",
            "PAdES + ML-DSA-65 signing",
            "Replay diff = 0",
        ],
        "kpis": ["K-03", "K-04", "K-08"],
    },
...
traceability = [
    {
        "feature": "M1 Phases + critical path",
        "control": "Phase-gate Rego + dep graph",
        "regimes": ["ISO 42001 Cl 6/9", "NIST RMF Govern"],
    },
    {
        "feature": "M2 AI Safety research",
        "control": "Hypothesis register + AISI joint",
        "regimes": ["EO 14110", "EU AI Act Art 55", "OECD AI Principles"],
    },
    {
        "feature": "M3 Global governance policy",
        "control": "Treaty + Codex + Cert + ICGC",
        "regimes": ["Council of Europe AI Convention", "G7 Hiroshima", "FSB"],
    },
    {
        "feature": "M4 Reference architecture",
        "control": "Terraform + EKS + Cilium + Kata",
        "regimes": ["EU AI Act Art 12/15", "DORA", "ISO 27001"],
    },
    {
        "feature": "M5 Governance dashboards",
        "control": "Board tile + MRM + kill-switch + portals",
        "regimes": ["FCA Consumer Duty", "EU AI Act Art 13", "ISO 42001 Cl 7.4"],
    },
    {
        "feature": "M6 Security + DevSecOps",
        "control": "Sigstore + OPA + zero-egress + WORM + judge",
        "regimes": ["SLSA L3+", "EU AI Act Art 15", "GDPR Art 32"],
    },
    {
        "feature": "M7 RAG governance",
        "control": "ACL + residency + taint + lineage",
        "regimes": ["GDPR Arts 5/6/32", "EU AI Act Art 10"],
    },
    {
        "feature": "M8 EAIP protocol",
        "control": "Envelope + signing + capability ticket",
        "regimes": ["EU AI Act Art 12", "FIPS 203/204"],
    },
    {
        "feature": "M9 CCaaS + PETs",
        "control": "DP + redaction + TEE + WORM",
        "regimes": ["FCA Consumer Duty", "GDPR Arts 6/22/25/32"],
    },
    {
        "feature": "M10 Prompt Architect",
        "control": "Templating + VCS + tests + refusal lattice",
        "regimes": ["EU AI Act Art 13", "FCA Consumer Duty", "GDPR Art 22"],
    },
    {
        "feature": "M11 Model registry",
        "control": "Manifest + lineage + tiering + evidence",
        "regimes": ["SR 11-7", "EU AI Act Annex IV", "PRA SS1/23"],
    },
    {
        "feature": "M12 Threat-intel + interp + telemetry",
        "control": "Feed + probes + OTel + lab",
        "regimes": ["NIST GAI Profile", "EU AI Act Art 15", "ISO 27001"],
    },
    {
        "feature": "M13 AGI/ASI sims",
        "control": "SRASE + CSE-X + AGI Lab + harness",
        "regimes": ["EU AI Act Art 55", "Treaty Annex", "EO 14110"],
    },
    {
        "feature": "M14 Report workflows",
        "control": "Auto pack + PAdES + zk + WORM",
        "regimes": [
            "EU AI Act Annex IV",
            "SR 11-7",
            "FCA Consumer Duty",
...
dataFlows = [
    {
        ...
        "controls": ["Rego unit tests", "ML-DSA-44", "Object Lock"],
    },
    {
        "id": "DF-02",
        "name": "Dependency graph build + critical path",
        "steps": [
            "import work items",
            "edges + durations",
            "topo sort",
        ...
        "controls": ["DAG validation", "Versioned snapshots"],
    },
    {
        "id": "DF-03",
        "name": "Annex IV auto-pack",
        "steps": [
            "registry pull",
            "MRM reports",
            "drift window",
        ...
        "controls": ["≤ 30 min SLA", "Replay diff = 0"],
    },
    {
        "id": "DF-04",
        "name": "Prompt Architect publish",
        "steps": [
            "author",
        ...
        "controls": ["κ ≥ 0.9", "Approval workflow"],
    },
    {
        "id": "DF-05",
        ...
        "controls": ["DLP", "Vector ACL", "CRS-UUID per chunk"],
    },
    {
        "id": "DF-06",
        "name": "Civilizational simulation drill",
        "steps": [
            "scenario load",
            "personas run",
            "composite score",
        ...
        "controls": ["≥ 0.9 gate", "PQC sign", "Independent assurance"],
    },
]
...
regulators = [
    ...
    },
    {
        "id": "REG-11",
        "name": "OECD + FSB + BIS",
        "primary": "International coordination",
    },
    {
        "id": "REG-12",
        "name": "Treaty Secretariat + UN",
        "primary": "Civilizational treaty",
    },
]
...
workshops = [
    {
        ...
        "duration": "2 h",
        "outcome": "Sign-off WP-050 phasing + budget envelope + Cert plan",
    },
    {
        "id": "WS-02",
        "audience": "C-Suite + SMFs + PMO",
        "duration": "1 d",
        "outcome": "Phase-gate operating model + OKR rollup + risk register",
    },
    {
        "id": "WS-03",
        "audience": "AI Research + AI Safety",
        "duration": "2 d",
        "outcome": "Research hypotheses + interp roadmap + AISI joint plan",
    },
    {
        "id": "WS-04",
        "audience": "Platform Eng + EA + Security",
        "duration": "2 d",
        "outcome": "Reference architecture rollout + DevSecOps pipeline",
    },
    {
        "id": "WS-05",
        "audience": "MRM + 2LoD + Compliance",
        "duration": "1 d",
        "outcome": "Model registry + Annex IV + SR 11-7 pack drills",
    },
    {
        "id": "WS-06",
        "audience": "UX + Frontend + Backend",
        "duration": "1 d",
        "outcome": "Dashboard catalogue + design system + API contracts",
    },
    {
        "id": "WS-07",
        "audience": "Treaty Liaison + AISI + Supervisor",
        "duration": "1 d",
        "outcome": "GIEN + Cert + sanctions ladder + Annex pipeline",
    },
]
...
rollout90 = [
    {
        "day": "0-30",
        "track": "P0 Foundations & Guardrails",
        "items": [
    ...
    {
        "day": "31-60",
        "track": "P1 RefArch + Dashboards Alpha",
        "items": [
            "OPA sidecar GA",
    ...
    {
        "day": "61-90",
        "track": "P1 Reports + Registry seed",
        "items": [
...
roadmap = [
    {
        "year": "2026",
        "focus": "P0-P2 Foundations + RefArch + Registry + Dashboards",
        "milestones": [
    ...
    {
        "year": "2027",
        "focus": "P3 Federation + Verifier + Sims",
        "milestones": [
    ...
    {
        "year": "2028",
        "focus": "Civilizational Operations Steady-State",
        "milestones": [
            "CSE-X 30+ scenarios",
            "Codex v1 ratified",
    ...
    {
        "year": "2029",
        "focus": "Maturity + Research Outputs",
        "milestones": [
            "Cert Platinum",
